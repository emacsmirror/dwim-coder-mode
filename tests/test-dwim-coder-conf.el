...
(defun dwim-coder-test-conf-mode ()
  (setq conf-indent-guess-indent-offset nil)
  (conf-unix-mode)
  (electric-pair-mode)
  (setq dwim-coder-auto-space nil)
  (dwim-coder-mode))

(defun dwim-coder-test-auto-space-conf-mode ()
  (setq conf-indent-guess-indent-offset nil)
  (conf-unix-mode)
  (electric-pair-mode)
    ...
    (dolist (elt files)
      (it (concat mode " - " (file-name-base (cdr elt)))
        ...
        (if (file-exists-p (concat in-file ".skip"))
          ...
          (setq result (test-buffer-with-content mode-func "test.conf" in-content))
...
(describe "dwim-coder-conf-basic"
  (let* ((file (concat (file-name-directory load-file-name) "conf/basic-tests"))
      ...
      (it line
        ...
        (if (not (string-match-p "^‖" line))
          ...
          (when (and expected-with-space
            ...
            (test-content in expected-with-space 'dwim-coder-test-auto-space-conf-mode "test.conf"))
          (test-content in expected 'dwim-coder-test-conf-mode "test.conf"))))))

(describe "dwim-coder-conf-main"
  (let* ((file (concat (file-name-directory load-file-name) "conf/main-tests"))
         (content (get-file-contents file))
          ...
          (dolist (item (cdr items))
            (cond
             ...
             ((string-match-p " *with-space" item)
              (setq func 'dwim-coder-test-auto-space-conf-mode))
             (t
            ...
            (if (not (bound-and-true-p func))
              ...
              (test-content in expected func "test.el")))))
      ))
  )
